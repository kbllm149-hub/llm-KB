...
model_names_str = ", ".join([f'"{name}"' for name in model_names])
# ---------- 基礎 Prompts（無 Auto-CoT 時使用） ----------

MULTI_MODEL_SYSTEM_PROMPT = f"""
請你扮演一位專業的山羊飼養與照護專家，對比所有 AI 模型的回答，判斷哪一個整體更好，並針對每個模型提供「回答正確性」評分。

你將會看到：
- 使用者問題（User Question）
- 參考答案（Reference Answer）
- 各個模型的回答 (Model Answer)

請依照以下流程進行：
1. 比較各模型的回答與參考答案，指出差異，並標註優缺點。
2. 為每個模型的回答，給出「回答正確性」評分（1–100 分，100 為最佳）。
3. 判斷哪一個模型的回答最正確、完整、實用。
...
STEPS_USER_PROMPT = "產生一組審查流程，專注於：與參考答案一致性、事實/安全檢核、關鍵要點覆蓋、邏輯與可操作性。"
...
為每個模型的回答，給出「回答正確性」評分（1–100 分，100 為最佳）。
最後必須從下列模型名稱中選出唯一優勝者（winner），名稱必須完全一致：
{model_names_str}
輸出只允許 JSON，格式如下：
{{
...
def judge_multi_model(system_prompt: str, user_q: str, ref_a: str, model_answers: Dict[str, str]) -> Dict[str, Any]:
    ...
    if isinstance(winner, str):
        if winner in model_answers.keys():
            parsed["winner"] = winner
        else:
            print(f"[Warning] LLM 輸出 winner 不在模型列表中：{winner}")
            parsed["winner"] = ""
    else:
        print("[Warning] LLM 輸出 winner 欄位型別不正確。")
        parsed["winner"] = ""
